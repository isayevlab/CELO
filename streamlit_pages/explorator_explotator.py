...
def explorator_explotator():
    ...
    n_clusters = st.slider("Number of clusters for diverse selection", min_value=1, max_value=sample_number, step=1)
    ...
    if experiment_name is not None:
        ...
        if os.path.exists(space_path) and os.path.exists(predictions_path):
            ...
            for i in range(n_clusters):
                points_per_cluster.append(sample_number // n_clusters + (i < sample_number % n_clusters))
            ...
            space.loc[reduced_space.index, "Color"] = "Explored"
            space.loc[selected.index, "Color"] = "Selected"
